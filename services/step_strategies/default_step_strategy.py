    ...
    def execute_step(self, job_id: str, job_info: Dict[str, Any], step: Dict[str, Any], 
        ...
        if current_step_index == 0:
            ...
            if report_text and isinstance(report_text, str) and report_text.strip():
                report_handler = ReportHandler(self.job_handler.blob_storage) if not hasattr(self.job_handler, 'report_handler') else self.job_handler.report_handler
                url = report_handler.save_report_to_blob(job_id, job_info, report_text, report_generated_by_agent=True)
