...
class RevisorBoardStepExecutor(BaseStepExecutor):
    ...
    def execute(self, job_id: str, job_info: Dict[str, Any], step: Dict[str, Any],
        ...
        if analysis_type == 'criacao_tarefas_azure_devops':
            print(f"[{job_id}] [DEBUG] RevisorBoardStepExecutor: analysis_type=criacao_tarefas_azure_devops, feature_id propagado: {agent_params['feature_id']}")
        print(f"[{job_id}] [DEBUG] Chamando agente revisor_board com epic_id={agent_params.get('epic_id')}, feature_id={agent_params.get('feature_id')}, task_id={agent_params.get('task_id')}")
        max_retries = 3
