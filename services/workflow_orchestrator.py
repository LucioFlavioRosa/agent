...
class WorkflowOrchestrator(IWorkflowOrchestrator):
    ...
    def execute_workflow(self, job_id: str, start_from_step: int = 0) -> None:
        ...
        workflow = self.workflow_registry.get(job_info['data'][JobFields.ORIGINAL_ANALYSIS_TYPE])
        if not workflow:
            raise ValueError("Workflow não encontrado.")
        try:
            repository_type = job_info['data'][JobFields.REPOSITORY_TYPE]
            repo_name = job_info['data'][JobFields.REPO_NAME]
    ...
    def _execute_step_with_strategy(self, job_id: str, job_info: Dict[str, Any], step: Dict[str, Any], 
        ...
        if is_comparador_agent:
            agent_params.update({
                'repo_name_modernizado': job_info['data'].get(JobFields.REPO_NAME_MODERNIZADO),
                'branch_name_modernizado': job_info['data'].get(JobFields.BRANCH_NAME_MODERNIZADO),
                'repo_name_original': job_info['data'].get(JobFields.REPO_NAME_ORIGINAL),
                'branch_name_original': job_info['data'].get(JobFields.BRANCH_NAME_ORIGINAL)
            })
        else:
            repo_name = job_info['data'].get(JobFields.REPO_NAME_MODERNIZADO, job_info['data'][JobFields.REPO_NAME])
            branch_name = job_info['data'].get(JobFields.BRANCH_NAME_MODERNIZADO, job_info['data'][JobFields.BRANCH_NAME])
            agent_params.update({
        ...
        agent_params.update({
            'usar_rag': job_info.get("data", {}).get(JobFields.USAR_RAG, False), 
