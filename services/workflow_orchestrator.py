...
class WorkflowOrchestrator(IWorkflowOrchestrator):
    def __init__(self, job_manager: IJobManager, blob_storage: IBlobStorageService,
                 ...
                 job_handler=None, report_handler=None, commit_handler=None, data_formatter=None,
                 secret_manager=None, cache_service=None, dependency_container=None,
                 epic_and_task_creation_service=None, llm_provider_factory: Optional[ILLMProviderFactory] = None,
                 repository_provider_factory: Optional[IRepositoryProviderFactory] = None,
                 commit_and_build_service=None, dotnet_build_service=None):
        self.workflow_registry = workflow_registry
        self.rag_retriever = rag_retriever or AzureAISearchRAGRetriever()
        self.job_handler = job_handler or job_manager
    ...
    def execute_workflow(self, job_id: str, start_from_step: int = 0) -> None:
        ...
        strategy = WorkflowExecutionStrategyFactory.create_strategy(
            ...
            self.job_handler,
            self.report_handler,
            self.epic_and_task_creation_service,
            self.commit_and_build_service,
            self.dependency_container.incremental_step_executor_service if self.dependency_container and hasattr(self.dependency_container, 'incremental_step_executor_service') else None,
