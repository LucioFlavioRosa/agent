...
def _fetch_missing_fields_from_blob(session_data, usuario_executor, nome_projeto, project_id):
    estado_blob = None
    try:
        if usuario_executor and nome_projeto:
            try:
                ...
                logging.getLogger("RedisSessionService").error(f"Erro ao buscar estado do Blob Storage para preencher campos: {str(e)}")
        if estado_blob and isinstance(estado_blob, dict):
            if not session_data.get("project_id") and estado_blob.get("project_id"):
                session_data["project_id"] = estado_blob["project_id"]
            if not session_data.get("usuario_executor") and estado_blob.get("usuario_executor"):
                session_data["usuario_executor"] = estado_blob["usuario_executor"]
            if not session_data.get("nome_projeto") and estado_blob.get("nome_projeto"):
                session_data["nome_projeto"] = estado_blob["nome_projeto"]
    except Exception as e:
        logging.getLogger("RedisSessionService").error(f"Erro inesperado ao tentar buscar campos do Blob: {str(e)}")
    return session_data
...
def ensure_project_id(session_data: dict, usuario_executor: str = None, nome_projeto: str = None) -> str:
    ...
    if project_id and isinstance(project_id, str) and project_id.strip():
        return project_id
    session_data = _fetch_missing_fields_from_blob(session_data, usuario_executor, nome_projeto, None)
...
class RedisSessionService:
    ...
    def create_session(self, usuario_executor: str, nome_projeto: str, analysis_type: str, project_id: str, extracted_text: Optional[str] = None, initial_state: Optional[Dict[str, Any]] = None) -> str:
        ...
        if not project_id or not isinstance(project_id, str) or not project_id.strip():
            ...
            session_data_temp = _fetch_missing_fields_from_blob(session_data_temp, usuario_executor, nome_projeto, None)
            project_id = session_data_temp.get("project_id")
            if not project_id:
