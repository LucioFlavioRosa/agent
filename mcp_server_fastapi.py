...
# Inicialização de containers e serviços
container = DependencyContainer(async_mode=True)  # Sinaliza uso de serviços assíncronos
api_service_factory = ApiServiceFactory(async_mode=True)
...
# Controle de concorrência para background tasks (Fase 2, mas já preparado para uso futuro)
import asyncio
workflow_semaphore = asyncio.Semaphore(8)  # Limite de 8 workflows concorrentes
...
async def run_workflow_task(job_id: str, start_from_step: int = 0):
    """Executa workflow em background com controle de concorrência."""
...
async def start_analysis(payload: StartAnalysisPayload, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(asyncio.create_task, run_workflow_task(job_id, start_from_step=0))
...
async def update_job_status(payload: UpdateJobPayload, background_tasks: BackgroundTasks):
    ...
    if payload.action == JobActions.APPROVE:
        ...
        background_tasks.add_task(asyncio.create_task, run_workflow_task(payload.job_id, start_from_step=start_from_step))
...
async def start_code_generation_from_report(analysis_name: str, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(asyncio.create_task, run_workflow_task(new_job_id, start_from_step=0))
