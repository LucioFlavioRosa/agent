    ...
    async def save_document(
        ...
        try:
            ...
            async with BlobServiceClient.from_connection_string(conn_str) as blob_service_client:
                ...
                file_size = len(file_data) if isinstance(file_data, bytes) else 'stream'
                logger.info(f"blob_upload_sucesso | blob_path={full_blob_path} | file_size={file_size}")
                
                return full_blob_path
                
        except Exception as e:
            logger.error(f"blob_upload_erro | blob_path={full_blob_path} | erro={e}")
            raise
    ...
    async def download_document(
        ...
        try:
            ...
            async with BlobServiceClient.from_connection_string(conn_str) as blob_service_client:
                ...
                logger.info(f"blob_download_sucesso | blob_path={blob_path} | file_size={len(file_bytes)}")
                return file_bytes
                
        except ResourceNotFoundError:
            logger.error(f"blob_download_erro | blob_path={blob_path} | motivo=arquivo_nao_encontrado")
            raise
        except Exception as e:
