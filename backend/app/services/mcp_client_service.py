...
class MCPClientService:
 ...
 self.logger = logging.getLogger("MCPClientService")
 ...
 url = f"{base}/start"
 self.logger.info(f"🔌 [MCP Client] URL Final Limpa: '[{url}]'")
 ...
 self.logger.info(f"Enviando arquivo .docx: {arquivo_docx.filename} (Content-Type: {arquivo_docx.content_type})")
 else:
 self.logger.info("Nenhum arquivo .docx incluído na requisição.")
 self.logger.info(f"Payload enviado ao MCP: {data}")
 ...
 self.logger.info(f"Resposta MCP: status_code={response.status_code}, body={response.text}")
 if response.status_code != 200:
 self.logger.error(f"❌ [MCP Client] Erro {response.status_code}: {response.text}")
 ...
 self.logger.error(f"❌ [MCP Client] HTTPStatusError ao chamar [{url}]: {exc.response.status_code} - {exc.response.text}", exc_info=True)
 raise Exception(f"Erro ao comunicar com MCP Server: {exc.response.status_code} - {exc.response.text}")
 except Exception as exc:
 self.logger.error(f"❌ [MCP Client] Falha ao chamar [{url}]: {str(exc)}", exc_info=True)
 ...
 self.logger.error(f"Erro ao buscar projetos do MCP: {str(exc)}")
 ...
 self.logger.error(f"Erro ao buscar relatório do MCP: {str(exc)}")
