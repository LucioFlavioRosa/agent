...
class ProjectStateService:
    ...
    async def _get_project_id_by_name(usuario_executor: str, nome_projeto: str) -> Optional[str]:
        ...
        if cache_key in ProjectStateService._project_id_cache:
            logger.info(f"[DEBUG] Cache hit para _get_project_id_by_name: {cache_key} → {ProjectStateService._project_id_cache[cache_key]}")
        ...
        logger.info(f"[DEBUG] Buscando project_id para usuario_executor='{usuario_executor}', nome_projeto='{nome_projeto}' (normalizado='{nome_projeto_normalizado}')")
        _, container_client = _get_blob_clients()
        prefix_resumo = f"{usuario_executor}/"
        blobs_resumo = list(container_client.list_blobs(name_starts_with=f"{prefix_resumo}"))
        for blob in blobs_resumo:
            if blob.name.endswith('.json') and "estado_resumo_" in blob.name:
                ...
                state_bytes = blob_client.download_blob().readall()
                state = json.loads(state_bytes.decode("utf-8"))
                nome_blob = state.get("nome_projeto", "")
                nome_blob_normalizado = nome_blob.strip().lower() if nome_blob else None
                if nome_blob_normalizado == nome_projeto_normalizado:
                    project_id = state.get("project_id")
                    if not project_id or not isinstance(project_id, str) or not project_id.strip():
                        project_id = ensure_project_id(state, usuario_executor, nome_blob)
                    ProjectStateService._project_id_cache[cache_key] = project_id
                    logger.info(f"[DEBUG] Encontrado project_id='{project_id}' para nome_projeto='{nome_projeto}'")
                    return project_id
        logger.info(f"[DEBUG] Nenhum project_id encontrado para nome_projeto='{nome_projeto}'")
        return None
    ...
    async def load_latest_state_from_blob(usuario_executor: str, project_id: Optional[str] = None, nome_projeto: Optional[str] = None, report_type: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        logger.info(f"[DEBUG] list_blobs prefix='{prefix}' project_id='{project_id}' nome_projeto='{nome_projeto}' report_type='{report_type}'")
        ...
        for blob in blobs:
            if blob.name.endswith('.json') and file_prefix in blob.name:
                ...
                if project_id and pid == project_id:
                    logger.info(f"[DEBUG] Estado encontrado para project_id='{project_id}' blob='{blob.name}'")
                    return state
                if nome_projeto and state.get("nome_projeto") == nome_projeto:
                    logger.info(f"[DEBUG] Estado encontrado para nome_projeto='{nome_projeto}' blob='{blob.name}'")
        ...
        if nome_projeto and states:
            ...
            logger.info(f"[DEBUG] Retornando estado mais recente para nome_projeto='{nome_projeto}'")
