...
class ProjectStateService:
    ...
    async def save_state_to_blob(session_data) -> str:
        ...
        for k in REPORT_FIELDS:
            logger.info(f"[save_state_to_blob] {k}: {json.dumps(state.get(k, {}), ensure_ascii=False)}")
    ...
    async def load_latest_state_from_blob(usuario_executor: str, projeto: str, session_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        for k in REPORT_FIELDS:
            if k not in state:
                ...
                missing_fields.append(k)
        for k in REPORT_FIELDS:
            logger.info(f"[load_latest_state_from_blob] {k}: {json.dumps(state.get(k, {}), ensure_ascii=False)}")
        return state
