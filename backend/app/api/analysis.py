...
async def start_analysis(
 ...
 "content_type": arquivo_docx.content_type,
 "size": None
 }
 arquivo_docx.file.seek(0, 2) # Move para o fim
 arquivo_info["size"] = arquivo_docx.file.tell()
 arquivo_docx.file.seek(0) # Volta para início
 logger.info(f"Arquivo .docx recebido: {arquivo_info}")
 except Exception as e:
 logger.warning(f"Falha ao obter informações do arquivo .docx: {e}")
 else:
 logger.info("Nenhum arquivo .docx recebido no request.")

 logger.info(f"Payload enviado ao MCP: {mcp_payload}")
