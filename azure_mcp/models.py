...
# Validadores específicos para campos obrigatórios nos workflows Azure
def validate_epic_id(value: Optional[str]) -> Optional[str]:
    if value is not None and not value.strip():
        raise ValueError('epic_id não pode ser vazio.')
    return value

def validate_feature_id(value: Optional[str]) -> Optional[str]:
    if value is not None and not value.strip():
        raise ValueError('feature_id não pode ser vazio.')
    return value

def validate_task_id(value: Optional[str]) -> Optional[str]:
    if value is not None and not value.strip():
