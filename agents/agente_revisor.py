        ...
        for i, step in enumerate(steps_to_run):
            ...
            if agent_response is None:
                ...
                if agent_type == "revisor":
                    ...
                    # [DEBUG] Validação dos dados recebidos para logging
                    print(f"[DEBUG] Dados recebidos para logging: projeto={projeto}, analysis_type={job_info['data']['original_analysis_type']}, llm_model={llm_model}, job_id={job_id}, status_update={status_update}")
                    
                    # Verificação de campos obrigatórios
                    if not projeto:
                        print(f"[WARNING] Campo 'projeto' está ausente ou vazio")
                    if not llm_model:
                        print(f"[WARNING] Campo 'llm_model' está ausente ou vazio")
                    if not job_id:
                        print(f"[WARNING] Campo 'job_id' está ausente ou vazio")
                    if not status_update:
                        print(f"[WARNING] Campo 'status_update' está ausente ou vazio")
