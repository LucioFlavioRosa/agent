# Arquivo: tools/github_reader.py (VERSÃO FINAL, ROBUSTA E OTIMIZADA COM DETECÇÃO DE REPO NOVO)
...
class GitHubRepositoryReader(IRepositoryReader):
    ...
    def is_repo_novo(self, nome_repo: str, nome_branch: str = None) -> bool:
        """
        ...
        try:
            ...
            if len(branches) > 1:
                return False
            commits = list(repositorio.get_commits(sha=branch_a_ler))
            if len(commits) > 2:
                return False
            contents = list(repositorio.get_contents("", ref=branch_a_ler))
            arquivos_relevantes = [c for c in contents if c.type == "file" and c.name.lower() != "readme.md"]
            if arquivos_relevantes:
                return False
            return True
        except Exception as e:
            print(f"[GitHubRepositoryReader] Erro ao detectar se o repo é novo: {e}")
            return False
