...
class RedisJobStore(JobStoreInterface):
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"ERRO CRÍTICO ao conectar ao Redis: {e}")
    ...
    def set_job(self, job_id: str, job_data: Dict[str, Any], ttl: int = 86400):
        ...
        if not self.redis_client:
            print(f"ERRO: Redis não está disponível. Job não será salvo: {key}")
            return
        try:
    ...
    def get_job(self, job_id: str) -> Optional[Dict[str, Any]]:
        key = f"{self.JOB_KEY_PREFIX}:{job_id}"
        if not self.redis_client:
            print(f"ERRO: Redis não está disponível. Não é possível buscar job: {key}")
            return None
        try:
    ...
    def set_analysis_name_index(self, job_id: str, analysis_name: str):
        ...
        """
        if not self.redis_client:
            print(f"ERRO: Redis não está disponível. Não é possível indexar analysis_name: {analysis_name}")
            return
        try:
    ...
    def get_job_by_analysis_name(self, analysis_name: str) -> Optional[Dict[str, Any]]:
        ...
        """
        if not self.redis_client:
            print(f"ERRO: Redis não está disponível. Não é possível buscar por analysis_name: {analysis_name}")
            return None
        try:
            analysis_name_normalized = analysis_name.lower()
