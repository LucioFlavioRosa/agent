...
class AzureRepositoryProvider(IRepositoryProvider):
    ...
    def get_repository(self, repository_name: str, token: str) -> Dict[str, Any]:
        try:
            ...
            # Garantir que o atributo 'id' esteja presente (pode ser 'id' ou 'repositoryId' dependendo da API)
            if 'id' not in repo_data:
                if 'repositoryId' in repo_data:
                    repo_data['id'] = repo_data['repositoryId']
                else:
                    raise ValueError(f"Resposta da API Azure DevOps não contém o campo 'id' ou 'repositoryId'. Dados: {repo_data}")
            # Garantir que todos os atributos necessários estejam presentes e não nulos
            missing_attrs = []
            for attr in ['_organization', '_project', 'id', '_provider_type']:
                if attr not in repo_data or repo_data[attr] is None:
                    missing_attrs.append(attr)
            if missing_attrs:
                raise ValueError(f"Objeto repo do Azure DevOps está malformado. Atributos ausentes ou None: "
                                 + ", ".join([f"{attr}={repo_data.get(attr, None)}" for attr in missing_attrs]))
            return repo_data
