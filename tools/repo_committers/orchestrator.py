...
def processar_branch_por_provedor(
    ...
    print(f"[ORCHESTRATOR][DEBUG] repository_type: {repository_type}, nome_branch: {nome_branch_sanitizado}, branch_de_origem: {branch_de_origem}, branch_alvo_do_pr: {branch_alvo_do_pr}, num_mudancas: {len(conjunto_de_mudancas) if conjunto_de_mudancas else 0}")
    resultado = None
    try:
        ...
        # Validação robusta das chaves obrigatórias
        obrigatorias = ["branch_name", "success", "pr_url", "commit_url"]
        for chave in obrigatorias:
            if chave not in resultado or resultado[chave] is None:
                print(f"[ERRO][orchestrator] Resultado do committer não contém a chave obrigatória '{chave}' ou está None: {json.dumps(resultado, default=str)}")
        ...
        print(f"[ERRO][orchestrator] Falha crítica ao processar branch para o provedor '{repository_type}': {exc}")
        raise
